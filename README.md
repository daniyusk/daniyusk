<!-- markdownlint-disable MD033 MD041 -->
<div align="center">
    <img src="https://capsule-render.vercel.app/api?type=waving&height=150&color=0:9D00FF,100:7F00FF&text=Daniyusk%20Github's&textBg=false&section=header&fontColor=FFFFFF" alt="Daniyusk's Github" />
    <br>
    <img src="https://readme-typing-svg.herokuapp.com?font=Bytesized&pause=1000&color=BFC4D4&center=true&vCenter=true&repeat=false&width=435&lines=In+constant+debug+%E2%80%94+of+life+and+code." alt="In constant debug - of life and code" />
</div>

---

<h2 align="center">✏️ ABOUT ME</h2>

<p align="center">
    I'm a developer who is always seeking new challenges, whether by learning or building. <br>
    When coding, I focus on creating well-structured and scalable architectures.
</p>

<p align="center">
    <strong>🎯 Goal:</strong> Use my skills to create meaningful solutions and positively impact people’s lives.
</p>

---

<h2 align="center">⚙️ TECHNOLOGIES & SKILLS</h2>

<div align="center">
    <p><strong>Languages</strong></p>
    <img src="https://skillicons.dev/icons?i=html,css,js,ts,python,java,kotlin,c,cpp,cs,haxe,lua,rust&perline=5" alt="Languages" />
    <br><br>
    <p><strong>Frameworks & Libraries</strong></p>
    <img src="https://skillicons.dev/icons?i=nodejs,nestjs,nextjs,react,vite,tailwind,tauri,prisma,discordjs,net,haxeflixel,selenium&perline=5" alt="Frameworks" />
    <br><br>
    <p><strong>Databases</strong></p>
    <img src="https://skillicons.dev/icons?i=mongodb,mysql,sqlite,postgres&perline=5" alt="Databases" />
    <br><br>
    <p><strong>Tools</strong></p>
    <img src="https://skillicons.dev/icons?i=docker,git,linux,vscode,visualstudio,eclipse,idea,androidstudio,gamemakerstudio,unity,godot,robloxstudio,figma,obsidian&perline=5" alt="Tools" />
</div>

---

<h2 align="center">📝 STATS</h2>

<div align="center">
    <img src="https://github-readme-streak-stats.herokuapp.com/?user=daniyusk&theme=react&hide_border=true" alt="daniyusk's Streak" height="150" />
    <img src="https://github-readme-activity-graph.vercel.app/graph?username=daniyusk&theme=react&hide_border=true" height="150" />
</div>

---

<h2 align="center">💫 GET IN TOUCH</h2>

<div align="center">
    <a href="https://www.linkedin.com/in/daniel-senzaki-132905407">
        <img src="https://skillicons.dev/icons?i=linkedin" width="50" alt="Linkedin" />
    </a>
    <a href="mailto:daniyusk.dev@gmail.com">
        <img src="https://skillicons.dev/icons?i=gmail" width="50" alt="E-mail" />
    </a>
    <a href="https://discord.com/users/670030102490382346">
        <img src="https://skillicons.dev/icons?i=discord" width="50" alt="Discord" />
    </a>
</div>

<div align="center">
    <img src="https://capsule-render.vercel.app/api?type=waving&height=150&color=0:9D00FF,100:7F00FF&textBg=false&section=footer&fontColor=FFFFFF">
</div>
